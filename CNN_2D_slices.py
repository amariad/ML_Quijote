...
#2D ROTATION FUNCTION for data augmentation

def random_rotation(dataset, rotation):
    
    if rotation ==0:
        choice = np.rot90(dataset, 1, (0,1)) #rotating along one axis only 
    elif rotation ==1:
        ...
        choice = np.rot90(np.fliplr(dataset), 1, (0,1)) #flip and rotate
    elif rotation ==5:
        choice = np.rot90(np.fliplr(dataset), 2, (0,1))
    elif rotation ==6:
        choice = np.rot90(np.fliplr(dataset), 3, (0,1))
    elif rotation ==7:
        choice = np.fliplr(dataset) # flip the identity    
    return choice
...
# 2D Constructor
    def __init__(self, out_1=4, out_2=8, out_3=32, out_4=64, out_5=128, out_6=64, params=2):
        ...
        self.cnn3 = nn.Conv2d(in_channels=out_2, out_channels=out_3, kernel_size=5, 
                              stride=3, padding=0)
        self.cnn4 = nn.Conv2d(in_channels=out_3, out_channels=out_4, kernel_size=5, 
                              stride=3, padding=0)
        self.cnn5 = nn.Conv2d(in_channels=out_4, out_channels=out_5, kernel_size=3, 
                              stride=7, padding=0)
        #self.cnn6 = nn.Conv2d(in_channels=out_5, out_channels=out_6, kernel_size=3, 
                              #stride=2, padding=0)

        self.BN1 = nn.BatchNorm2d(num_features=out_1)
        self.BN2 = nn.BatchNorm2d(num_features=out_2)
        self.BN3 = nn.BatchNorm2d(num_features=out_3)
        self.BN4 = nn.BatchNorm2d(num_features=out_4)
        self.BN5 = nn.BatchNorm2d(num_features=out_5)
        #self.BN6 = nn.BatchNorm2d(num_features=out_6)
        ...
        self.fc1 = nn.Linear(out_5 * 2 * 2, 250) 
...
offset=0
total_choice = 8 						#number of available rotaions
num_rot=7 							#number of rotations to apply
...
class Data_set(Dataset):
	...
	#read all 2D images
        self.images = np.zeros((length*num_rot, 256, 256), dtype=np.float32)
        count = 0 
        for i in range(length):
            f_df = '/mnt/ceph/users/fvillaescusa/Ana/density_field_2D/latin_hypercube/%d/df_z=0.npy'%(i)
            df = np.load(f_df)
            for j in range(num_rot):
                df_rotated = random_rotation(df, rotations[i,j])
                self.images[count] = df_rotated
                count += 1
        min_df = np.min(self.images)
        max_df = np.max(self.images)
        self.images = (self.images - min_df)/(max_df - min_df) #normalize the images
        self.images = torch.tensor(self.images)
        self.len = length*num_rot
        
    def __getitem__(self, index):
        return self.images[index].unsqueeze(0), self.cosmo_params[index]
...
fout   = 'losses/2d_losses__5cnn_3fcl_b99_7_rots.txt'      	#losses for plotting 	RENAME
fmodel = 'best_models/2d_5cnn_3fcl_b99_7_rots.pt'   		#best model		RENAME 
...
test_results   = 'predictions/results_2d_5cnn_3fcl_b99_7_rots_11_24.txt'  		#RENAME
